...
function Header() {
  ...
  const avatarRef = useRef(null);    // avatar ref

  const navigate = useNavigate();
  const location = useLocation();
  const currentPath = location.pathname;
  const isInnerCourse = location.pathname.startsWith("/course/");
  ...
  const linkClass = (path) =>
    `hover:text-[#F97316] transition ${currentPath === path ? "text-[#F97316] " : ""
    }`;
  ...
  return (
    <nav
      className={`${isInnerCourse ? "relative" : "fixed top-0"
          ...
          bg-white dark:bg-black shadow-md  h-[50px]
        `}
    >
...
>
  ...
  <Link to="/blog" className={linkClass("/blog")}>Blog</Link>
      ...
      {/* Right Side */}
      {login ? (
        <div className="w-full lg:w-[auto] pr-6 sm:pr-5 hidden sm:flex items-center justify-end gap-2 relative">
        ...
        <div className="md:flex hidden pr-3  text-sm items-center">
          <div className="mr-4">
            <Toggle />
          </div>
